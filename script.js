...
// NEU: Verbindung zum neuen Render-Server
...
socket.onmessage = (event) => {
    ...
    if (data.type === 'match_found') {
        myColor = data.color;
        addChat("System", "Match gefunden! Du spielst " + (myColor === 'white' ? "Weiß" : "Schwarz"), "other");
    }
};
...
function sendMessage() {
    ...
    if (text !== "" && socket.readyState === WebSocket.OPEN) {
        ...
        const type = mode === "global_chat" ? "global_chat" : "chat";
        socket.send(JSON.stringify({ type: type, text: text, sender: "WeltGast" }));
        addChat("Du", text, "me");
        chatInput.value = "";
    }
}
...
document.getElementById("connectMP").onclick = () => {
    ...
    if (mode === "random") {
        socket.send(JSON.stringify({ type: 'find_random', name: 'WeltGast' }));
        addChat("System", "Suche Gegner...", "other");
    } else {
        socket.send(JSON.stringify({ type: 'join', room: room || "global", name: 'WeltGast' }));
        addChat("System", "Raum " + (room || "global") + " beigetreten", "other");
    }
};

// --- SCHACH LOGIK (UNVERÄNDERT) ---
function resetGame() {
    board = [
