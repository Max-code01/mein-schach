...
// --- WEBSOCKET EMPFANG ---
socket.onopen = () => {
    // Beim Start dem globalen Raum beitreten
...
socket.onmessage = (event) => {
    ...
    // Züge von anderen empfangen
    if (data.type === 'move') {
        const m = data.move;
        // Führe den Zug aus, aber sende ihn nicht zurück (verhindert Endlosschleife)
        executeRemoteMove(m.fr, m.fc, m.tr, m.tc);
    }
};
...
// Hilfsfunktion für Züge von anderen (damit kein Loop entsteht)
function executeRemoteMove(fr, fc, tr, tc) {
