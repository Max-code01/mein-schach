...
function addChat(sender, text, type) {
    ...
    // Hier ist das Zahnrad-Icon für Systemmeldungen
    m.innerHTML = type === "system" ? `⚙️ ${text}` : `<strong>${sender}:</strong> ${text}`;
    chatMessages.appendChild(m);
    chatMessages.scrollTop = chatMessages.scrollHeight;
}

// --- 3. SERVER LOGIK (LEADERBOARD FIX) ---
socket.onmessage = (e) => {
    ...
    switch(d.type) {
        ...
        case 'user-count':
            ...
            break;
        case 'leaderboard':
            // FIX: Hier wird die Bestenliste jetzt korrekt befüllt
            if (leaderboardEl) {
                leaderboardEl.innerHTML = d.list.map((p, i) => 
                    `<div style="display:flex; justify-content:space-between; padding:3px 0; border-bottom:1px solid rgba(255,255,255,0.05)">
                        ...
                        <span style="color:var(--warning)">${p.wins} 🏆</span>
                    </div>`).join('');
            }
            break;
    }
...
gameModeSelect.onchange = () => {
    ...
    if (mode === "random") {
        ...
        addChat("System", "Bot-Modus (Smart) gestartet. Viel Erfolg!", "system");
    } else {
        addChat("System", "Modus auf Lokal (2 Spieler) gewechselt.", "system");
    }
};

document.getElementById("connectMP").onclick = () => {
    const r = document.getElementById("roomID").value || "global";
    addChat("System", `Verbinde mit Raum ${r}...`, "system");
        ...
        row.forEach((p, c) => {
            ...
            d.onclick = () => {
                ...
                if(selected) {
                    ...
                    if((gameModeSelect.value === "online" || gameModeSelect.value === "random") && !isOwn(board[r][c], myColor)) return;
                    selected = {r, c};
                }
                draw();
            };
...
document.getElementById("resetBtn").onclick = () => { addChat("System", "Neues Spiel gestartet.", "system"); resetGame(); };
document.getElementById("resignBtn").onclick = () => { addChat("System", "Spiel aufgegeben.", "system"); resetGame(); };
