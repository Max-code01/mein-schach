/* ===== ENGINE WORKER MIT RUST-POWER (TIEFE 4) ===== */

// Import der WebAssembly-Brücke zur logic.rs
import init, { evaluate_board, get_position_bonus } from './pkg/logic.js';

let wasmReady = false;
init().then(() => {
    wasmReady = true;
    console.log("🦀 Rust-Logik (logic.rs) erfolgreich geladen!");
});
...
// VERBESSERT: Nutzt jetzt die logic.rs für die Bewertung
function evaluate(board) {
    if (wasmReady) {
        // Wir wandeln das Board in einen String für Rust um
        const boardStr = board.flat().map(p => p === "" ? "." : p).join("");
        return evaluate_board(boardStr); // Aufruf der Rust-Funktion
    }

    // Fallback falls Rust noch lädt
    let score = 0;
    for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) {
            const p = board[r][c];
